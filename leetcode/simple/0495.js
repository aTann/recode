...
// 单次扫描
// 会归零，累计 Math.min(duration, next-pre) 即可
// O(n)/(1)
// 56ms/44.3MB
var findPoisonedDuration = function (timeSeries, duration) {
  ...
  for (let i = 1; i < len; i++) {
    let interval = timeSeries[i] - timeSeries[i - 1];
    ans += Math.min(duration, interval);
  }

  return ans;
};
var findPoisonedDuration = function (timeSeries, duration) {
  let i = timeSeries.length;
  let ans = duration;
